...
parse_templates = (subs, tenv) ->
	components =
		...
		template: {once: {}, line: {}, word: {}, syl: {}, char: {}}
	...
	for i, line in ipairs subs
		...
		if classifier == 'once' and line_type != 'code' and line_type != 'template'
			error 'The `once` classifier is only valid on `code` or `template` lines.'
		...
		component =
			...
			line: nil -- present on `template once` lines
		...
		if line_type == 'template' and classifier == 'once'
			component.line = line
			-- forcing notext as the template line will also be its own input (meaning text would be double-inserted)
			-- this means, among other things, that `mixin char` will not work, which is unfortunate
			-- possibly, the template version should be cleaned to only take the text as "input"?
...
-- Somewhat hacky: collect all the self-referential "input" lines for `template once` and append to existing `lines`
-- Doing this ensures they will be preprocessed by karaskel (though the usefulness of this is likely quite limited)
collect_once_input = (lines, components) ->
	table.insert lines, template.line for template in *components.template.once
...
apply_templates = (subs, lines, components, tenv) ->
	...
	run_once_templates = () ->
		for template in *components.template.once
			tenv.orgline = template.line
			run_template 'once', template, template.line
			tenv.orgline = nil
