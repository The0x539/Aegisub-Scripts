...
#### `no_t_actor`
<img src="https://www.mariowiki.com/images/thumb/2/27/SuperMarioParty_Waluigi.png/420px-SuperMarioParty_Waluigi.png" height="100">
...
Only valid for `template`s. Primarily useful for `syl`.

Make the template apply per-highlight in multi-highlight timed karaoke.
During execution, `orgsyl` will reference each of the highlights, while the true original `syl` can be accessed as `morgsyl`.

```
template syl multi: !retime("syl",0,0)!{\an5\t(0,80,\fscx150\fscy150)\t(80,0,\fscx100\fscy100)}
kara: {\k34}Pulse{\k20}#{\k78}#
```

The timed karaoke line uses basic multi-highlight markup, the `#` syllables, to create multi-highlight syllables. Such, `Pulse` is a single syllable with three highlights, `Pulse` and `#` twice. The `#` characters aren't displayed at all in the applied effect.

Note that `retime` is necessary here to ensure that the output lines have the correct timing. Without it, both output lines would have the same timing as the input line.
...
More information on common fields can be found [here](https://aegisub.org/docs/latest/automation/lua/modules/karaskel.lua/#dialogue-line-table).
